...
conn2 = pyodbc.connect('DRIVER=SQL Server;SERVER=UKSALAZSQL;DATABASE=Sandbox;Trusted_Connection=Yes;UID=SALITIX\SQLSalitixAuditorUsers')
cursor2 = conn2.cursor()

conn3 = pyodbc.connect('DRIVER=SQL Server;SERVER=UKSALAZSQL;DATABASE=Sandbox;Trusted_Connection=Yes;UID=SALITIX\SQLSalitixAuditorUsers')
cursor3 = conn3.cursor()

promo_schedule_path=r"C:\Users\Python\Desktop\Morrisons Analysis Project\Promo_Schedules"

Salitix_customer_number_0="MOR01"

cursor2.execute("DELETE [Sandbox].[dbo].[Promotions_Morrisons_Stg];")
cursor2.commit()

for i in os.listdir(promo_schedule_path):
    if i[:1]=="M":
        df=pd.read_csv(os.path.join(promo_schedule_path,i))
        Salitix_client_name_0=i.replace("MORRISONS_PROMO_SCHEDULE_","")
        ...
        for x,row in df.iterrows():
            ...
            row_info_0[2]="'"+row_info_0[2].replace(".0","")+"'"
            ...
            print(row_info)
            sql1 = "INSERT INTO [Sandbox].[dbo].[Promotions_Morrisons_Stg] (" +cols + ") VALUES ("+row_info+");"
